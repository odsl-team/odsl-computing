...
## Applying for an MPCDF account

To get access to the ODSL compute resources at MPCDF go the the [MPCDF "new users"](https://www.mpcdf.mpg.de/userspace/new-users) page and fill out the application form:

Select "MPI for Physics" as your institution, select "Oliver Schulz" as the application reviewer, select cost center "0" and "Lukas Heinrich" supervisor (under Account details).

Also familiarize yourself with the [MPCDF technical documentation](https://docs.mpcdf.mpg.de/).

Once your account is fully ready, perform the following steps to set thing up.
...
More interactive compute nodes will be added, hopefully soon:
